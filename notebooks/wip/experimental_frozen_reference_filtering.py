...
def _(CLUSTER_OCCUPANCY_CUTOFF, FROZEN_CENTERS, MATCH_RADIUS, mo):
    ...
    - **Pareto knee precision/recall/F1** — the max-F1 per-structure point, scored
      against the frozen reference. This is a **moving argmax** — a different
      structure may win each row — so it carries no monotonicity guarantee.
    - **default-knee precision/recall/F1** — the *same* structure (the unfiltered
      max-F1 knee) tracked across every filter. With the reference frozen this one
      structure's **recall is monotone non-increasing** by construction; a precision
      drop directly counts on-center waters the filter removed from it.
    """)
    return
...
def _(
    ...
    _FIELDS = [
        ...
        ("default_knee_precision", "default-knee precision", "float"),
        ("default_knee_recall", "default-knee recall", "float"),
        ("default_knee_f1", "default-knee F1", "float"),
    ]
    ...
    for _label, _col, _op, _thr in FILTERS:
        ...
        summaries[_label] = frozen_reference_summary(_fm, CLUSTER_OCCUPANCY_CUTOFF)

    # The default row's best-F1 structure, tracked across every filter. Because the
    # reference is frozen, this one structure's recall is monotone non-increasing by
    # construction; any precision drop counts on-center waters the filter removed
    # from it. (The moving "Pareto knee" columns above re-pick the argmax structure
    # each row, so they carry no such guarantee.)
    _default_pr = summaries["default"]["pr_df"]
    DEFAULT_KNEE_PDB = _default_pr.loc[_default_pr["f1"].idxmax(), "pdb_id"]

    def _default_knee(pr_df):
        hit = pr_df[pr_df["pdb_id"] == DEFAULT_KNEE_PDB]
        if hit.empty:
            return {
                "default_knee_precision": None,
                "default_knee_recall": None,
                "default_knee_f1": None,
            }
        row = hit.iloc[0]
        return {
            "default_knee_precision": float(row["precision"]),
            "default_knee_recall": float(row["recall"]),
            "default_knee_f1": float(row["f1"]),
        }

    _rows = []
    for _label, *_rest in FILTERS:
        _combined = {**summaries[_label], **_default_knee(summaries[_label]["pr_df"])}
        _row = {"variant": _label}
        for _field, _header, _kind in _FIELDS:
            _row[_header] = _fmt(_combined[_field], _kind)
        _rows.append(_row)

    summary_table = pd.DataFrame(_rows).set_index("variant")
    summary_table
    return DEFAULT_KNEE_PDB, summaries, summary_table


@app.cell
def _(DEFAULT_KNEE_PDB, mo):
    mo.md(f"""
    *The **default-knee** columns track structure **`{DEFAULT_KNEE_PDB}`*** "
        "— the unfiltered max-F1 structure — across every filter row. A `—` means "
        "that structure lost all its waters under the filter.*
    """)
    return
    ...
    def build_pr_grid(filters, summaries, *, panel=3.2, fontsize=11):
        ...
        for ax, (label, *_rest) in zip(axes[0], filters):
            ...
            plot_pr_scatter(
                ...
                cbar_kwargs={"fraction": 0.03, "aspect": 30, "pad": 0.05},
            )
            ...
            ax.text(0.04, 0.04,
                    f"knee P = {knee['precision']:.2f}\nknee R = {knee['recall']:.2f}",
                    transform=ax.transAxes, ha="left", va="bottom",
                    fontsize=fontsize - 1, color="r", zorder=6)
...
def _(
    ...
    def build_quality_split_fig(members, clusters, cutoff, filters, *, fontsize=11):
        ...
        fig, axes = plt.subplots(1, len(specs), figsize=(3.5 * len(specs), 3))
