...
For every ordered pair (a, b) of cohort members, aligns b onto a (Cα Kabsch) and
compares their water oxygens — a is the reference / ground truth, b the predicted
set. One row per ordered pair → data/<cohort_id>/pairwise_metrics_{cutoff}.csv.

Metrics: precision/recall/f1 (many-to-one), matched precision/recall (one-to-one)
and chamfer distance. The cutoff is applied uniformly, so it lives in the filename
rather than a column; a failed alignment leaves NaN in rmsd_after and the metric
columns.
"""
...
def main() -> None:
    ...
    parser.add_argument("cohort", type=Path, nargs="?", help="Cohort .txt file")
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=None,
