...
def zinc_positions(model) -> list:
    return [a.pos for chain in model for res in chain for a in res if a.element.name == "Zn"]
...
def _residue_has_sulfonamide(res) -> bool:
    ...
    for s in (a for a in atoms if a.element.name == "S"):
        n_o = sum(
            1 for a in atoms if a is not s and a.element.name == "O" and s.pos.dist(a.pos) <= 1.65
        )
        n_n = sum(
            1 for a in atoms if a is not s and a.element.name == "N" and s.pos.dist(a.pos) <= 1.95
        )
        if n_o >= 2 and n_n >= 1:
            return True
    return False
...
def has_sulfonamide(model, comp_ids: set[str]) -> bool:
    return any(
        res.name in comp_ids and _residue_has_sulfonamide(res) for chain in model for res in chain
    )
