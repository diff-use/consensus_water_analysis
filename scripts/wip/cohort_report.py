...
def stage_artifacts(
    cohort: Path, protein: str, reference: str, outdir: Path, force: bool
) -> dict[str, Path]:
    ...
    stale_metadata = (
        ...
        and "diffrn_temp" not in pd.read_csv(paths["metadata"], nrows=0).columns
    )
    if force or not paths["metadata"].exists() or stale_metadata:
        ...
        run(
            [python, "scripts/build_metadata.py", str(cohort), "-o", str(paths["metadata"])],
            "metadata",
        )
    if force or not paths["apo_holo"].exists():
        run(
            [
                python,
                f"scripts/wip/{PROTEIN_SCRIPTS[protein]}",
                "--cohort",
                str(cohort),
                "--csv",
                str(paths["apo_holo"]),
    ...
    if force or not paths["mutations"].exists():
        run(
            [
                python,
                "scripts/wip/mutation_survey.py",
                str(cohort),
                "--reference",
                reference,
                "-o",
...
def build_table(paths: dict[str, Path], cohort_ids: list[str], reference: str) -> pd.DataFrame:
    ...
    table = (
        ...
        .merge(
            mutations[["pdb_id", "n_substitutions", "identity", "signature"]],
            on="pdb_id",
            how="left",
        )
    )
    ...
    if paths["text"].exists():
        ...
        table = table.merge(
            text.rename(columns={"verdict": "text_verdict"}), on="pdb_id", how="left"
        )
    else:
    ...
    table["ligand_class"] = table.ligand_state.map(
        lambda c: (
            "unknown" if not isinstance(c, str) else ("holo" if c.startswith("holo") else "apo")
        )
    )
...
def report(table: pd.DataFrame, cohort_id: str, resolution_cutoff: float | None) -> None:
    ...
    print(
        f"  {'ligand':<10}{'sequence':<12}{'temp':<16}{'n':>5}   {'med.res':>8}{'med.waters':>12}"
    )
    ...
    for (ligand, sequence, temperature), group in grouped:
        print(
            f"  {ligand:<10}{sequence:<12}{temperature:<16}{len(group):>5}   "
    ...
    if resolution_cutoff is not None:
        steps.append(
            (f"+ resolution <= {resolution_cutoff:g} A", table.resolution <= resolution_cutoff)
        )
    ...
    print()
    print(
        "  Note: the cascade is one ordering of the same criteria. Reorder or relax any "
...
def main() -> None:
    ...
    parser.add_argument(
        "--protein",
        required=True,
        choices=sorted(PROTEIN_SCRIPTS),
        help="Which apo_holo_*.py defines the active site",
    )
    parser.add_argument(
        "--reference", required=True, help="Reference PDB ID for the mutation survey"
    )
    parser.add_argument(
        "-o",
        "--output-dir",
